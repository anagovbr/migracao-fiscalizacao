...
DELETE FROM FISCALIZACAO.FISTB_MULTAOCORRENCIA
WHERE MUO_AOH_CD IN (
    SELECT AOH_CD
    FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA
    WHERE AOH_OCO_CD IN (
        SELECT OCO_CD
        FROM FISCALIZACAO.FISTB_OCORRENCIA
        WHERE OCO_IC_MIGRADO = 1
...
DELETE FROM FISCALIZACAO.FISTB_RECURSOOCORRENCIA
WHERE REO_AOH_CD IN (
    SELECT AOH_CD
    FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA
    WHERE AOH_OCO_CD IN (
...
DELETE FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA
WHERE AOH_OCO_CD IN (
    SELECT OCO_CD
    FROM FISCALIZACAO.FISTB_OCORRENCIA
    WHERE OCO_IC_MIGRADO = 1
...
BEGIN
  FOR t IN (
    ...
    SELECT 'FISCALIZACAO_ETL_RW.FISTB_INSTRUMENTOFISCALIZACAO_TEMP' FROM DUAL UNION ALL
    SELECT 'FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP' FROM DUAL UNION ALL
    SELECT 'FISCALIZACAO_ETL_RW.FISTB_MULTAOCORRENCIA_TEMP' FROM DUAL UNION ALL
    SELECT 'FISCALIZACAO_ETL_RW.FISTB_RECURSOOCORRENCIA_TEMP' FROM DUAL
  ) LOOP
    BEGIN
...
CREATE TABLE FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP AS SELECT * FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA WHERE 1=0;
CREATE TABLE FISCALIZACAO_ETL_RW.FISTB_MULTAOCORRENCIA_TEMP AS SELECT * FROM FISCALIZACAO.FISTB_MULTAOCORRENCIA WHERE 1=0;
CREATE TABLE FISCALIZACAO_ETL_RW.FISTB_RECURSOOCORRENCIA_TEMP AS SELECT * FROM FISCALIZACAO.FISTB_RECURSOOCORRENCIA WHERE 1=0;
...
SELECT
  ...
  0, -- AUI_IC_LAVRADOEMCAMPO ('Instrumento lavrado em campo = Não' para toda a importação.) Alteração na regra de negócio pelo Josimar no dia 06/01/2026.
  ...
  CASE WHEN s.OCORRENCIANUMERODOCREF IS NOT NULL THEN s.OCORRENCIANUMERODOCREF ELSE NULL END, -- AUI_NU_DOCUMENTOREFERENCIA
  CASE WHEN s.OCORRENCIANUMERODOCREF IS NOT NULL THEN 1 ELSE 0 END, -- AUI_IC_VINCULACAOAUIANTERIOR
  CASE
  ...
  s.OCORRENCIANUMERODOC, -- AUI_NU_INSTRUMENTO. Segundo o Josimar, é para excluir ANO-REFERENCIA que ele havia pedido anteriormente.
...
SELECT
  (SELECT NVL(MAX(AOH_CD), 0) FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA) + ROW_NUMBER() OVER (ORDER BY o.OCO_CD),
  NVL(s.OCORRENCIADATAPAGAMENTO, s.OCORRENCIADATAINICIO),
  7, -- Multa Ajustada
  NULL,
  'Dados de multa migrados do sistema legado',
  o.OCO_CD,
  o.OCO_TSO_CD
FROM FISCALIZACAO_ETL_RW.FISTB_OCORRENCIA_TEMP o
JOIN FISCALIZACAO_ETL_RW.STG_TBOCORRENCIA s ON o.IDOCORRENCIA = s.IDOCORRENCIA
WHERE (s.OCORRENCIAVALORMULTA IS NOT NULL
   OR s.OCORRENCIAVALORPAGO IS NOT NULL
   OR s.OCORRENCIADATAPAGAMENTO IS NOT NULL);

-- 5B. Insere dados de multa em FISTB_MULTAOCORRENCIA
INSERT INTO FISCALIZACAO_ETL_RW.FISTB_MULTAOCORRENCIA_TEMP (
  MUO_CD,
  MUO_IC_MUDANCA_VALOR,
  MUO_NU_VALOR,
  MUO_DT_PAGAMENTO,
  MUO_IC_PAGAMENTO_TOTAL,
  MUO_DS_OBSERVACAO,
  MUO_AOH_CD,
  MUO_DT_PAGAMENTO_GRU,
  MUO_NU_VALOR_PG
)
SELECT
  (SELECT NVL(MAX(MUO_CD), 0) FROM FISCALIZACAO.FISTB_MULTAOCORRENCIA) + ROW_NUMBER() OVER (ORDER BY o.OCO_CD),
  0,
  s.OCORRENCIAVALORMULTA,
  s.OCORRENCIADATAPAGAMENTO,
  NULL,
  NULL,
  aoh.AOH_CD,
  s.OCORRENCIADATAFINALMULTA,
  s.OCORRENCIAVALORPAGO
FROM FISCALIZACAO_ETL_RW.FISTB_OCORRENCIA_TEMP o
JOIN FISCALIZACAO_ETL_RW.STG_TBOCORRENCIA s ON o.IDOCORRENCIA = s.IDOCORRENCIA
JOIN FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP aoh
  ON aoh.AOH_OCO_CD = o.OCO_CD AND aoh.AOH_TOP_CD = 7
WHERE (s.OCORRENCIAVALORMULTA IS NOT NULL
   OR s.OCORRENCIAVALORPAGO IS NOT NULL
   OR s.OCORRENCIADATAPAGAMENTO IS NOT NULL);

-- 5C. Insere FISTB_ACOMPANHAMENTOOCORRENCIA para Recurso 1a Instância
INSERT INTO FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP (
  AOH_CD,
  AOH_DT_OPERACAO,
  AOH_TOP_CD,
...
SELECT
  (SELECT NVL(MAX(AOH_CD), 0) FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA) + (SELECT COUNT(*) FROM FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP) + ROW_NUMBER() OVER (ORDER BY o.OCO_CD),
  s.RECURSO1DTRECEBIMENTO,
  5, -- Recurso Registrado
  NULL,
  'Recurso de 1a Instância migrado do sistema legado',
  o.OCO_CD,
  o.OCO_TSO_CD
FROM FISCALIZACAO_ETL_RW.FISTB_OCORRENCIA_TEMP o
JOIN FISCALIZACAO_ETL_RW.STG_TBOCORRENCIA s ON o.IDOCORRENCIA = s.IDOCORRENCIA
WHERE s.RECURSO1DTRECEBIMENTO IS NOT NULL;

-- 5D. Insere FISTB_ACOMPANHAMENTOOCORRENCIA para Recurso 2a Instância
INSERT INTO FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP (
  AOH_CD,
  AOH_DT_OPERACAO,
  AOH_TOP_CD,
...
SELECT
  (SELECT NVL(MAX(AOH_CD), 0) FROM FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA) + (SELECT COUNT(*) FROM FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP) + ROW_NUMBER() OVER (ORDER BY o.OCO_CD),
  s.RECURSO2DTRECEBIMENTO,
  5, -- Recurso Registrado
  NULL,
  'Recurso de 2a Instância migrado do sistema legado',
  o.OCO_CD,
  o.OCO_TSO_CD
FROM FISCALIZACAO_ETL_RW.FISTB_OCORRENCIA_TEMP o
JOIN FISCALIZACAO_ETL_RW.STG_TBOCORRENCIA s ON o.IDOCORRENCIA = s.IDOCORRENCIA
WHERE s.RECURSO2DTRECEBIMENTO IS NOT NULL;

-- 5E. Insere dados de Recurso 1a Instância
INSERT INTO FISCALIZACAO_ETL_RW.FISTB_RECURSOOCORRENCIA_TEMP (
  REO_CD,
  REO_DT_INTERPOSICAO,
  REO_NU_PROTON_PARECER,
  REO_NU_PROTON_OFICIO,
  REO_SIR_CD,
  REO_IC_ALTERACAO_AUTO,
  REO_DS_OBSERVACAO,
  REO_AOH_CD,
  REO_REC_CD,
  REO_DT_CIENCIAUSUARIO,
  REO_TSO_CD_ANTERIOR,
  REO_DT_LIMITE,
  REO_IC_TEMPESTIVO
)
SELECT
  (SELECT NVL(MAX(REO_CD), 0) FROM FISCALIZACAO.FISTB_RECURSOOCORRENCIA) + ROW_NUMBER() OVER (ORDER BY o.OCO_CD),
  s.RECURSO1DTRECEBIMENTO,
  NULL,
  NULL,
  CASE
    WHEN s.IDSITUACAORECURSO1 = 1 THEN 1 -- Deferido
    WHEN s.IDSITUACAORECURSO1 = 2 THEN 2 -- Indeferido
    WHEN s.IDSITUACAORECURSO1 = 3 THEN 3 -- Parcialmente deferido
    WHEN s.IDSITUACAORECURSO1 = 4 THEN 4 -- Não conhecido
    ELSE NULL
  END,
  NULL,
  -- Se recurso de 2a instância não existir, observação gravada na 1a instância.
  CASE
    WHEN s.RECURSO2DTRECEBIMENTO IS NULL THEN SUBSTRB(s.RECURSOOBSERVACAO, 1, 4000)
    ELSE NULL
  END,
  aoh.AOH_CD,
  1, -- 1a Instância
  NULL,
  NULL,
  s.RECURSO1PRAZOFINAL,
  NULL
FROM FISCALIZACAO_ETL_RW.FISTB_OCORRENCIA_TEMP o
JOIN FISCALIZACAO_ETL_RW.STG_TBOCORRENCIA s ON o.IDOCORRENCIA = s.IDOCORRENCIA
JOIN FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP aoh
  ON aoh.AOH_OCO_CD = o.OCO_CD
  AND aoh.AOH_TOP_CD = 5
  AND aoh.AOH_DS_NOTAS = 'Recurso de 1a Instância migrado do sistema legado'
WHERE s.RECURSO1DTRECEBIMENTO IS NOT NULL;

-- 5F. Insere dados de Recurso 2a Instância
INSERT INTO FISCALIZACAO_ETL_RW.FISTB_RECURSOOCORRENCIA_TEMP (
  REO_CD,
  REO_DT_INTERPOSICAO,
  REO_NU_PROTON_PARECER,
...
SELECT
  (SELECT NVL(MAX(REO_CD), 0) FROM FISCALIZACAO.FISTB_RECURSOOCORRENCIA) + (SELECT COUNT(*) FROM FISCALIZACAO_ETL_RW.FISTB_RECURSOOCORRENCIA_TEMP) + ROW_NUMBER() OVER (ORDER BY o.OCO_CD),
  s.RECURSO2DTRECEBIMENTO,
  NULL,
  NULL,
  CASE
    WHEN s.IDSITUACAORECURSO2 = 1 THEN 1 -- Deferido
    WHEN s.IDSITUACAORECURSO2 = 2 THEN 2 -- Indeferido
    WHEN s.IDSITUACAORECURSO2 = 3 THEN 3 -- Parcialmente deferido
    WHEN s.IDSITUACAORECURSO2 = 4 THEN 4 -- Não conhecido
    ELSE NULL
  END,
  NULL,
  SUBSTRB(s.RECURSOOBSERVACAO, 1, 4000), -- Se recurso de 2a instância existir, observação é guardada na 2a instância.
  aoh.AOH_CD,
  2, -- 2a Instância
  NULL,
  NULL,
  s.RECURSO2PRAZOFINAL,
  NULL
FROM FISCALIZACAO_ETL_RW.FISTB_OCORRENCIA_TEMP o
JOIN FISCALIZACAO_ETL_RW.STG_TBOCORRENCIA s ON o.IDOCORRENCIA = s.IDOCORRENCIA
JOIN FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP aoh
  ON aoh.AOH_OCO_CD = o.OCO_CD
  AND aoh.AOH_TOP_CD = 5
  AND aoh.AOH_DS_NOTAS = 'Recurso de 2a Instância migrado do sistema legado'
WHERE s.RECURSO2DTRECEBIMENTO IS NOT NULL;
...
INSERT INTO FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA
SELECT
  AOH_CD, AOH_DT_OPERACAO, AOH_TOP_CD, AOH_DS_SERVIDOR, AOH_DS_NOTAS,
  AOH_OCO_CD, AOH_TSO_CD_MOMENTO
FROM FISCALIZACAO_ETL_RW.FISTB_ACOMPANHAMENTOOCORRENCIA_TEMP;

INSERT INTO FISCALIZACAO.FISTB_MULTAOCORRENCIA
SELECT
  MUO_CD, MUO_IC_MUDANCA_VALOR, MUO_NU_VALOR, MUO_DT_PAGAMENTO,
  MUO_IC_PAGAMENTO_TOTAL, MUO_DS_OBSERVACAO, MUO_AOH_CD,
  MUO_DT_PAGAMENTO_GRU, MUO_NU_VALOR_PG
FROM FISCALIZACAO_ETL_RW.FISTB_MULTAOCORRENCIA_TEMP;

INSERT INTO FISCALIZACAO.FISTB_RECURSOOCORRENCIA
SELECT
  REO_CD, REO_DT_INTERPOSICAO, REO_NU_PROTON_PARECER, REO_NU_PROTON_OFICIO,
  REO_SIR_CD, REO_IC_ALTERACAO_AUTO, REO_DS_OBSERVACAO, REO_AOH_CD,
  REO_REC_CD, REO_DT_CIENCIAUSUARIO, REO_TSO_CD_ANTERIOR, REO_DT_LIMITE,
  REO_IC_TEMPESTIVO
FROM FISCALIZACAO_ETL_RW.FISTB_RECURSOOCORRENCIA_TEMP;
...
BEGIN
  ...
  sync_sequence('FISSQ_ACOMPOCORRENCIA', 'FISCALIZACAO.FISTB_ACOMPANHAMENTOOCORRENCIA', 'AOH_CD');
  sync_sequence('FISSQ_MULTAOCORRENCIA', 'FISCALIZACAO.FISTB_MULTAOCORRENCIA', 'MUO_CD');
  sync_sequence('FISSQ_RECURSOOCORRENCIA', 'FISCALIZACAO.FISTB_RECURSOOCORRENCIA', 'REO_CD');

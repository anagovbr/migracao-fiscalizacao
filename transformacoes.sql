...
BEGIN
  FOR t IN (
    SELECT 'TEMP_SNISB_MAP' AS table_name FROM DUAL UNION ALL
    SELECT 'TEMP_CAMPANHA_MAP' FROM DUAL UNION ALL
    SELECT 'FISTB_CAMPANHA_TEMP' FROM DUAL UNION ALL
    SELECT 'FISTB_VISTORIA_BARRAGEM_TEMP' FROM DUAL UNION ALL
    SELECT 'FISTB_OCORRENCIA_TEMP' FROM DUAL UNION ALL
    SELECT 'FISTB_INSTRUMENTOFISCALIZACAO_TEMP' FROM DUAL
  ) LOOP
    BEGIN
...
CREATE TABLE FISTB_CAMPANHA_TEMP AS SELECT * FROM FISTB_CAMPANHA WHERE 1=0;
CREATE TABLE FISTB_VISTORIA_BARRAGEM_TEMP AS SELECT * FROM FISTB_VISTORIA_BARRAGEM WHERE 1=0;
CREATE TABLE FISTB_OCORRENCIA_TEMP AS SELECT * FROM FISTB_OCORRENCIA WHERE 1=0;
ALTER TABLE FISTB_OCORRENCIA_TEMP ADD (IDOCORRENCIA NUMBER);
CREATE TABLE FISTB_INSTRUMENTOFISCALIZACAO_TEMP AS SELECT * FROM FISTB_INSTRUMENTOFISCALIZACAO WHERE 1=0;

-- Cria uma tabela temporária para guardar o Id da campanha.
-- O conceito de campanha não existe nos dados do SQL Server.
-- CREATE TABLE TEMP_CAMPANHA_MAP (
--    IDVISTORIA NUMBER,
--    CAM_CD     NUMBER
-- );
CREATE TABLE TEMP_CAMPANHA_MAP (
  ...
  IDVISTORIA NUMBER,
  IDBARRAGEM NUMBER,
  IDCAMPANHA VARCHAR2(8) -- Vem da tabela vistoria do SQL Server
);
...
CREATE TABLE TEMP_SNISB_MAP AS
SELECT BAR_CD, BAR_CD_SNISB
FROM SNISSBARRAGENS.SNBFT_BARRAGEM@ORAHMG_LINK;
...
INSERT INTO TEMP_CAMPANHA_MAP (CAM_CD, IDVISTORIA, IDBARRAGEM, IDCAMPANHA)
SELECT 
  (SELECT NVL(MAX(CAM_CD), 0) FROM FISCALIZACAO_MIGRACAO.FISTB_CAMPANHA) + DENSE_RANK() OVER (ORDER BY v.IDBARRAGEM, v.IDCAMPANHA) AS CAM_CD,
  v.IDVISTORIA,
  v.IDBARRAGEM,
  v.IDCAMPANHA
FROM STG_TBVISTORIA v
JOIN STG_VW_BDBARRAGEM b ON v.IDBARRAGEM = b.IDBARRAGEM; -- Filtra apenas as barragens operadas pela ANA.
...
FROM (
  ...
  FROM TEMP_CAMPANHA_MAP m
  JOIN STG_TBVISTORIA v ON m.IDVISTORIA = v.IDVISTORIA
  GROUP BY m.CAM_CD, m.IDBARRAGEM, m.IDCAMPANHA
) c;
...
SELECT
  (SELECT NVL(MAX(VIB_CD), 0) FROM FISCALIZACAO_MIGRACAO.FISTB_VISTORIA_BARRAGEM) + ROW_NUMBER() OVER (ORDER BY v.IDVISTORIA) AS VIB_CD, -- VIB_CD
  n.BAR_CD, -- VIB_BAR_CD
  m.CAM_CD, -- VIB_CAM_CD
  NULL, -- VIB_NU
...
FROM STG_TBVISTORIA v
JOIN TEMP_CAMPANHA_MAP m ON v.IDVISTORIA = m.IDVISTORIA
...
SELECT
  (SELECT NVL(MAX(OCO_CD), 0) FROM FISCALIZACAO_MIGRACAO.FISTB_OCORRENCIA) + ROW_NUMBER() OVER (ORDER BY s.IDOCORRENCIA), -- OCO_CD
...
SELECT
  (SELECT NVL(MAX(AUI_CD), 0) FROM FISCALIZACAO_MIGRACAO.FISTB_INSTRUMENTOFISCALIZACAO) + ROW_NUMBER() OVER (ORDER BY s.IDOCORRENCIA), -- AUI_CD
...
-- 6. Copia das tabelas temporárias para as tabelas finais
...
FROM FISCALIZACAO_MIGRACAO.FISTB_CAMPANHA_TEMP;
...
FROM FISCALIZACAO_MIGRACAO.FISTB_VISTORIA_BARRAGEM_TEMP;
...
FROM FISCALIZACAO_MIGRACAO.FISTB_OCORRENCIA_TEMP;
...
FROM FISCALIZACAO_MIGRACAO.FISTB_INSTRUMENTOFISCALIZACAO_TEMP;

-- 7. Sincroniza as sequencias
DECLARE
  l_max_value NUMBER;
  l_seq_exists NUMBER;
